  ...
  DW.PROTOCOLS = [
    ...
    },
    {
      ...
      help: 'Décodage J1939 au-dessus de CAN : l’identifiant 29 bits est découpé en ' +
            ...
            'protocole de transport — voir « Messages multi-trames ».',
      linkFields: [
        ...
        F('tp', 'Messages multi-trames', 'enum', 'bam', false,
          'Comment obtenir les PGN de plus de 8 octets. Écoute des BAM : strictement ' +
          'passif — la source annonce puis diffuse son transfert, on se contente de le ' +
          'réassembler. Requêtes périodiques : le serveur RÉCLAME les PGN sur le bus et ' +
          'complète le dialogue point à point ; c’est le seul mode où il émet, à ' +
          'n’activer qu’en connaissance de cause. Mono-trame : ignorer les transferts.',
          { choices: [['off', 'Mono-trame seulement'], ['bam', 'Écoute des BAM (passif)'],
                      ['request', 'Requêtes périodiques (émet sur le bus)']] }),
        F('requestPeriodS', 'Période des requêtes (s)', 'int', 5, false,
          'Intervalle entre deux salves de demandes. Trop court, on charge le bus et le ' +
          'calculateur interrogé pour rien.', { when: { tp: ['request'] } }),
        F('ownSa', 'Notre adresse source', 'int', 249, false,
          'Adresse J1939 sous laquelle le serveur émet ses requêtes. 249 est réservée à ' +
          'un outil de diagnostic externe ; elle ne doit surtout pas être déjà portée par ' +
          'un calculateur du réseau.', { when: { tp: ['request'] } }),
      ],
      pointFields: [
