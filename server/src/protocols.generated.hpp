...
inline const std::vector<ProtocolDesc>& protocols_desc() {
  ...
  { "snmp", "SNMP", "UDP (port 161)",
    "live", "Gestionnaire SNMP en lecture seule : interrogation cyclique d’OID par GetRequest. Les trois versions sont lues — v1, v2c et v3 (USM : authentification MD5, SHA-1 ou SHA-256, chiffrement DES ou AES-128). Les phrases secrètes de v3 ne sont jamais dans la configuration : elle ne porte qu’une référence, résolue dans l’environnement du serveur. Un serveur compilé sans Net-SNMP sert encore v1 et v2c, et annonce v3 « non branché » plutôt que de retomber en clair. Aucune écriture (SetRequest) n’est possible.",
    {
    { "host", "Hôte", "text", "", true, "Adresse IP ou nom réseau de l’agent SNMP.", "" },
    { "port", "Port", "int", "161", false, "Port UDP de l’agent (161 par défaut).", "" },
    { "version", "Version", "enum", "v2c", true, "v1 : la plus ancienne, sans Counter64 ni exception par variable. v2c : le choix courant, mais communauté EN CLAIR sur le réseau. v3 : authentification et chiffrement (modèle USM) — le seul choix raisonnable sur un réseau exposé. v3 demande un serveur compilé avec Net-SNMP ; sans lui, le lien refuse de s’ouvrir plutôt que de retomber en clair.", "v1|v2c|v3" },
    ...
    { "secretRef", "Référence des secrets", "text", "", false, "Nom désignant les phrases secrètes — jamais les phrases elles-mêmes : cette configuration est lisible par tout poste connecté et s’exporte en clair. Le serveur lit DIAGWEB_SECRET_<RÉFÉRENCE>_AUTH et …_PRIV dans son environnement (à défaut DIAGWEB_SECRET_<RÉFÉRENCE> pour les deux), que systemd sait alimenter depuis son magasin de secrets sans rien écrire sur disque.", "" },
    ...
    { "tsOid", "OID d’horodatage", "text", "", false, "SNMP ne transporte AUCUNE date : ni la requête ni la réponse n’en portent. Certaines MIB en exposent une dans un objet voisin — indiquer ici son OID le fait lire dans la même requête, donc au même instant que la valeur. Laisser vide si la MIB n’en propose pas : l’horloge du serveur sera utilisée.", "" },
    { "tsType", "Type de l’horodatage", "enum", "dateAndTime", false, "Sans effet si aucun OID d’horodatage n’est indiqué. DateAndTime (RFC 2579) : date absolue, éventuellement avec son fuseau — la seule vraiment fiable. TimeTicks : centièmes de seconde depuis le démarrage de l’agent, ramenés en absolu par approximation.", "dateAndTime|timeTicks" },
    { "gain", "Gain", "float", "1", false, "Facteur appliqué à la valeur brute : valeur = brut × gain + décalage.", "" },
    { "offset", "Décalage", "float", "0", false, "Constante ajoutée après le gain (unité physique).", "" },
    { "timestamp", "Horodatage", "enum", "source", false, "Équipement : la date que l’équipement attache lui-même à la donnée — la seule fidèle quand l’événement précède sa transmission. Fournie par IEC 60870-5-104 (types horodatés), IEC 61850 (GOOSE, Sampled Values, rapports) et OPC UA en abonnement ; SNMP la tient d’un OID d’horodatage de la MIB, s’il en existe un (champ « OID d’horodatage » du point) ; ailleurs, l’horloge du serveur est utilisée de toute façon. Serveur : ignorer délibérément l’horodatage du protocole — à choisir quand l’horloge de l’équipement n’est pas de confiance.", "source|server" },
    } },
